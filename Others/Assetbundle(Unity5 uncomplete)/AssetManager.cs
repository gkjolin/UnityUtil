...
public class AssetManager : MonoBehaviour
{
	...
	IEnumerator LoadManifest ()
	{
		...
		Debug.Log ("スタートロード");
		SetDebugText ("Loading Manifest");
		using (WWW www = new WWW (pathToBundles + platform)) {
			yield return www;
			if (!string.IsNullOrEmpty (www.error)) {
				Debug.Log (www.error);
				gameObject.GetComponent<AssetBundleLoader> ().loadingText.text = "ネットワークに接続していません。";

				SetDebugText (www.error);
				return false;
			}
		...
		if (!isReady) {
			...
			SetDebugText ("There was an error loading manifest");
			Debug.Log ("Manifestをロード出来ませんでした");
		} else {
			Debug.Log ("Manifest loaded successfully");
			SetDebugText ("Manifest loaded successfully");
	...
	{
		////////print ("bundleNamea" + bundleName);
		//////////print ("assetmanager loadbundlecoroutine" + bundleName);
		...
		SetDebugText ("Finished loading " + bundleName);
	}

	void OnDisable ()
	{
		Debug.Log ("Unloading Bundles");
		SetDebugText ("Unloading Bundles");
